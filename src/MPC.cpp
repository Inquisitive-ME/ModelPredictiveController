 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      ...
      if(t<N-1)
      {
        fg[0] += weight_steer_use * CppAD::pow(vars[delta_start + t], 2);
        fg[0] += weight_accel_use * CppAD::pow(vars[a_start + t], 2);
      }

      // Minimize the value gap between sequential actuations.
      if(t<N-2)
      {
        fg[0] += weight_delta_steer * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
